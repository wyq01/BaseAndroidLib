...
import com.ts.base.printer.jqPrinter.cpcl.CPCL;
import com.ts.base.printer.jqPrinter.esc.ESC;
import com.ts.base.printer.jqPrinter.jpl.JPL;
...
public class JQPrinter {
     ...
     * 走纸到右黑标
     */
    public boolean feedRightMark() throws IOException {
        switch (param.model) {
     ...
     * 走纸到左黑标
     */
    public boolean feedLeftMark() throws IOException {
        switch (param.model) {
            case JLP351: //JLP351系列没有右黑标传感器，只能使用中间的标签缝传感器代替有右黑标传感器
            case JLP351_IC:
                _cmd[0] = 0x0C;
     ...
     * 获取打印机状态
     */
    public boolean getPrinterState(int timeout_read) throws IOException {
