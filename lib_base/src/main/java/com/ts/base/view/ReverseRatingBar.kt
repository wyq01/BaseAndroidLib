 ...
 */
    ...
    init {
        ...
        stepSize = StepSize.fromStep(
            mTypedArray.getInt(
                R.styleable.RatingBar_stepSize,
                1
            )
        )
    ...
    private fun setStar(rating: Float) {
        ...
        val b2 = BigDecimal(Integer.toString(ratingInt))
        //浮点数的小数部分
        val fPoint = b1.subtract(b2).toFloat()

        //设置选中的星星
        for (i in starCount - 1 downTo starCount - ratingInt) {
            (getChildAt(i) as ImageView).setImageDrawable(starFillDrawable)
        }
        //设置没有选中的星星
        for (i in 0 until starCount - ratingInt) {
            (getChildAt(i) as ImageView).setImageDrawable(starEmptyDrawable)
        }
        //小数点默认增加半颗星
        if (fPoint > 0) {
            (getChildAt(ratingInt) as ImageView).setImageDrawable(starHalfDrawable)
        }
    }
