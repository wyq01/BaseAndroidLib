...
import com.wyq.base.util.ChineseCalendarUtil;
...
public class DateTimePickerView extends LinearLayout implements NumberPickerView.OnValueChangeListener {
...
//    private int minute;
    ...
    private void initInternal(Context context) {
        View contentView = inflate(context, R.layout.base_view_date_time_picker, this);
...
//        minute = calendar.get(Calendar.MINUTE);
    ...
    private void setDisplayValuesForAll(ChineseCalendar cc, boolean isGregorian, boolean anim) {
        ...
        initValuesForHour(cc, anim);
        initValuesForMinute(cc, anim);
    }
    ...
    public void onValueChange(NumberPickerView picker, int oldVal, int newVal) {
        ...
        if (picker == mYearPickerView) {
            ...
            if (mOnDateChangedListener != null) {
                ...
                mOnDateChangedListener.onDateChanged(calendarData, isOutOfDate);
            }
    ...
    private void passiveUpdateMonthAndDay(int oldYearFix, int newYearFix, boolean isGregorian) {
        ...
        int hour = mHourPickerView.getValue();
        int minute = mMinutePickerView.getValue();
        ...
        if (isGregorian) {
            ...
            if (oldDayStop == newDayStop) {
                if (mOnDateChangedListener != null) {
                    CalendarData calendarData = getCalendarData(newYearFix, newMonthSway, oldDaySway, hour, minute, isGregorian);
                    mOnDateChangedListener.onDateChanged(calendarData, isOutOfDate);
                }
                return;
            }
            ...
            if (mOnDateChangedListener != null) {
                CalendarData calendarData = getCalendarData(newYearFix, newMonthSway, newDaySway, hour, minute, isGregorian);
                mOnDateChangedListener.onDateChanged(calendarData, isOutOfDate);
            }
            return;
        } else {
    ...
    private void passiveUpdateDay(int oldYear, int newYear, int oldMonth, int newMonth, boolean isGregorian) {
        ...
        if (oldDayStop == newDayStop) {
            if (mOnDateChangedListener != null) {
                CalendarData calendarData = getCalendarData(newYear, newMonth, oldDaySway, hour, minute, isGregorian);
                mOnDateChangedListener.onDateChanged(calendarData, isOutOfDate);
            }
            return;//不需要更新
        } else {
            ...
            if (mOnDateChangedListener != null) {
                CalendarData calendarData = getCalendarData(newYear, newMonth, newDaySway, hour, minute, isGregorian);
                mOnDateChangedListener.onDateChanged(calendarData, isOutOfDate);
            }
            return;
        }
    ...
    public void setGregorian(boolean isGregorian, boolean anim) {
        ...
        cc.set(Calendar.HOUR_OF_DAY, mHourPickerView.getValue());
        cc.set(Calendar.MINUTE, mMinutePickerView.getValue());
    ...
    private CalendarData getCalendarData(int pickedYear, int pickedMonthSway, int pickedDay,
                                         int pickedHour, int pickedMinute, boolean mIsGregorian) {
        if (!mIsGregorian) {
            long ymd = pickedYear * 10000 + pickedMonthSway * 100 + pickedDay;
            isOutOfDate = ymd > 21001201;
        }
        return new CalendarData(pickedYear, pickedMonthSway, pickedDay, pickedHour, pickedMinute, mIsGregorian);
    }
    ...
    public CalendarData getCalendarData() {
        ...
        int pickedMinute = mMinutePickerView.getValue();
        if (!mIsGregorian) {
            long ymd = pickedYear * 10000 + pickedMonthSway * 100 + pickedDay;
            isOutOfDate = ymd > 21001201;
        }
    ...
    public static class CalendarData {
        ...
        public CalendarData(int pickedYear, int pickedMonthSway, int pickedDay,
                            int pickedHour, int pickedMinute, boolean isGregorian) {
        ...
        private void initChineseCalendar() {
            ...
            chineseCalendar.set(Calendar.HOUR_OF_DAY, pickedHour);
            chineseCalendar.set(Calendar.MINUTE, pickedMinute);
        }
    ...
    public interface OnDateChangedListener {
        void onDateChanged(CalendarData calendarData, boolean isOutOfDate);
    }
