...
import com.wyq.base.printer.jqPrinter.common.ImageConvert;
...
public class Image extends BaseESC {
    ...
    // / 4)Y方向点数 Y_BYTES * 8
    // / </SUMMARY>
    private boolean userImageDownloadIntoRAM(final int x_bytes, final int y_bytes, final byte[] data) throws IOException {
        if (x_bytes <= 0)
            return false;
    ...
    // / 1)打印机不打印输出图像
    // / </summary>
    private boolean userImageDrawout(final ESC.IMAGE_ENLARGE mode) throws IOException {
        _cmd[0] = 0x1D;
        _cmd[1] = 0x2F;
    ...
    // / </summary>
    private boolean _drawOut(int image_width_dots, int image_height_dots, ESC.IMAGE_ENLARGE mode, byte[] image_data) throws IOException {
    ...
    private boolean _drawOut(int image_width_dots, int image_height_dots, ESC.IMAGE_ENLARGE mode, char[] image_data) throws IOException {
        int Y_Byte = (image_height_dots - 1) / 8 + 1; // 位图Y轴方向象素点的字节素；
        int X_Byte = (image_width_dots - 1) / 8 + 1; // 位图X轴方向象素点的字节素，表示需要X_Byte幅8
        // x BmpHeight的位图拼成目标位图；
        byte[] DotsBuf = new byte[Y_Byte * 8]; // 存放8 x BmpHeight位图的点阵数据；
    ...
    // / </summary>
    public boolean drawOut(int x, int y, int image_width_dots, int image_height_dots, ESC.IMAGE_ENLARGE mode, byte[] image_data) throws IOException {
        if (!setXY(x, y))
            return false;
    ...
    public boolean drawOut(int x, int y, int image_width_dots,
                           int image_height_dots, ESC.IMAGE_ENLARGE mode, char[] image_data) throws IOException {
        if (!setXY(x, y))
            return false;
        return _drawOut(image_width_dots, image_height_dots, mode, image_data);
    }
    ...
    // / 2)由于图像并没有立即输出，还可以继续在相应的x,y坐标绘制打印对象
    // / </summary>
    public boolean drawOut(int x, int y, Bitmap bitmap) throws IOException {
    ...
    // / 2)由于图像并没有立即输出，还可以继续在相应的x,y坐标绘制打印对象
    // / </summary>
    public boolean drawOut(int x, int y, String image_path) throws IOException {
    ...
    // / 2)图像立即输出,不能在图像上绘制文字
    // / </summary>
    public boolean printOut(int x, Bitmap bitmap, int sleep_time) throws IOException {
        int width = bitmap.getWidth();
        int height = bitmap.getHeight();
    ...
    // / 2)图像立即输出,不能在图像上绘制文字
    // / </summary>
    public boolean printOut(int x, String image_path, int sleep_time) throws IOException {
        if (new File(image_path).exists()) {
            Bitmap bitmap = BitmapFactory.decodeFile(image_path);
    ...
    private boolean _printOut(int x, int width, int height,
                              ESC.IMAGE_MODE mode, byte[] data, int sleep_time) throws IOException {
        if (width > this._param.escPageWidth) {
            return false;
        }
    ...
    public boolean printOutFast(int x, String image_path, int sleep_time,
                                int base_image_height) throws IOException {
        if (new File(image_path).exists()) {
            Bitmap bitmap = BitmapFactory.decodeFile(image_path);
     ...
     * int id,图像在资源中的id
     */
    public boolean printOutFast(int x, Resources res, int id) throws IOException {
    ...
    private boolean _printOutFast(int x, int width, int height, int enlargeX,
                                  int enlargeY, byte[] data, int sleep_time, int base_image_height) throws IOException {
        if (width > this._param.escPageWidth) {
            return false;
        }
     ...
     * 根据位图资源的id打印位图
     */
    public boolean printOut(int x, Resources res, int id, int sleep_time) throws IOException {
        Bitmap bitmap = BitmapFactory.decodeResource(res, id);
        int width = bitmap.getWidth();
        int height = bitmap.getHeight();
        if (width > this._param.escPageWidth)
    ...
    public void drawBitmap(int x, int y, Bitmap bitmap) throws IOException {
        ...
        byte[] data = imageConvert.CovertImageVertical(bitmap, 128, 8);

        if (data == null)
            return;
        setXY(x, y);
        _drawOut(width, height, ESC.IMAGE_ENLARGE.NORMAL, data);
    }
