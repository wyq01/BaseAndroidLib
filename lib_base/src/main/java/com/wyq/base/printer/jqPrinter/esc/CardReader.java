...
public class CardReader extends BaseESC {
    ...
    private boolean send_and_wait(CARD_CMD cmd_flag, byte[] req, int req_len, String from) throws IOException {
        rsp_len = 0;
        int len = 1 + req_len;
        _port.flushReadBuffer();
    ...
    /// 检测大卡座是否有卡插入
    public boolean checkBigCardInsert(byte[] state) throws IOException {
    ...
    public boolean setCardType(int channel, ESC.CARD_TYPE_MAIN card_type_main, int card_type_sub) throws IOException {
        req[0] = (byte) channel;
    ...
    /// CPU卡复位
    public boolean cardCPU_Reset(int channel) throws IOException {
        req[0] = (byte) channel;
