...
public class Text extends BaseCPCL {
     ...
     * 根据字体family限定字体size
     */
    public int getFontSize(FONT_FAMILY family, int size) {
        if (size < 0) size = 0;
        switch (family) {
            case ASCII_Standard:
                if (size > 6) size = 6;
    ...
    public boolean _setFontSetBold(boolean bBold) throws IOException {
        int bold = bBold ? 1 : 0;
        String CPCLCmd = "SETBOLD " + bold;
        return portSendCmd(CPCLCmd);
    }

    private boolean _setFontSetUnderline(boolean underline) throws IOException {
        String CPCLCmd;
        if (underline)
    ...
    //设置字体倍高倍宽
    public boolean _setFontSetMagCPCL(int ex, int ey) throws IOException {
    ...
    private boolean _drawOut(int x, int y, String text, int fontFamily, int fontSize, int ex, int ey, boolean bold, boolean underline) throws IOException {
...
//		if (!_setFontSetMagCPCL(ex, ey)) return false;
     ...
     * 绘制水平方向字符串
     */
    private boolean _drawOutHorizontal(int x, int y, String text, int family, int size) throws IOException {
