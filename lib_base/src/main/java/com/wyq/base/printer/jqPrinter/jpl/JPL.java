...
public class JPL {
     ...
     * 走纸到下一张标签开始
     */
    public boolean feedNextLabelBegin() throws IOException {
        _cmd[0] = 0x1A;
        _cmd[1] = 0x0C;
        _cmd[2] = 0x00;
    ...
    private boolean feed(FEED_TYPE feed_type, int offset) throws IOException {
        _cmd[0] = 0x1A;
        _cmd[1] = 0x0C;
        _cmd[2] = 0x01;
    ...
    //进入JPL模式
    public boolean intoGPL(int timerout_read) throws IOException {
    ...
    //退出JPL模式
    public boolean exitGPL(int timerout_read) throws IOException {
        _port.flushReadBuffer();
        _cmd[0] = 0x1a;
        _cmd[1] = 0x23;
        _cmd[2] = 0x00;
     ...
     */
    public boolean feedBack(int dots) throws IOException {
        return feed(FEED_TYPE.BACK, dots);
    }

    public boolean feedNextLabelEnd(int dots) throws IOException {
        return feed(FEED_TYPE.LABEL_END, dots);
    }

    public boolean feedMarkOrGap(int dots) throws IOException {
        return feed(FEED_TYPE.MARK_OR_GAP, dots);
    }

    public boolean feedMarkEnd(int dots) throws IOException {
        return feed(FEED_TYPE.MARK_END, dots);
    }

    public boolean feedMarkBegin(int dots) throws IOException {
