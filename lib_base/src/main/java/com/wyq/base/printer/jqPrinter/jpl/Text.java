...
public class Text extends BaseJPL {
    ...
    public boolean drawOut(int x, int y, String text, int fontHeight,
                           ...
                           JPL.ROTATE rotateAngle) throws IOException {
        if (x < 0 || y < 0)
            return false;
    ...
    private int calcTextStartPosition(Printer_define.ALIGN align, int startX, int endX, int font_height, int enlargeX, String text) {
        if (endX < startX) {
            int t = endX;
            endX = startX;
            startX = t;
     ...
     * 根据x坐标起点和结束点对齐文字
     */
    public boolean drawOut(Printer_define.ALIGN align, int startX, int endX, int y,
                           String text, int fontHeight, boolean bold, boolean reverse,
                           boolean underLine, boolean deleteLine, TEXT_ENLARGE enlargeX,
                           TEXT_ENLARGE enlargeY, JPL.ROTATE rotateAngle) throws IOException {
